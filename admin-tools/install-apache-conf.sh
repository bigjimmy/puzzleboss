...
cat ${PB_PATH}/apache-conf/sites/canadia-apache.conf-template | perl -pi -e 'my $instpath="'${INSTALL_PATH}'"; s/\$DOMAIN_NAME/'${DOMAIN_NAME}'/g; s/\$INSTALL_PATH/$instpath/g; s/\$BIGJIMMY_IP/'${BIGJIMMY_IP}'/g;' > ${PB_PATH}/apache-conf/sites/canadia-apache.conf
