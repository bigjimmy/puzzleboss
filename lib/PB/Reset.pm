...
sub registration : StartRunmode {
    ...
    if ($entriesok) {
        ...
        $html .=
          ...
          . $q->submit(
            -name => 'verifyregister',
            value => 'Everything looks good. Proceed with reset.'
          )
          . $q->p
          . $q->submit(
            -name => 'changes',
        ...
        $html .=
            $q->p
          . "Please enter your username and the e-mail used to register the account (for verification)."
          . $q->start_form(-method => 'POST')
          . $q->dl
          . $q->dt
          . "E-mail Address:"
          ...
          . $q->p
          . $q->submit(-name => 'register', -value => 'Reset My Password')
          . $q->hidden(-name => 'rm', -value => $nextrm, -override => 1)
          . $q->end_form;
    }
        ...
        if ($matchcount < 1) {
          ...
          # no duplication, we are ok to proceed to email validate this new user
                ...
                if (
                    !tie(
                        %hash, 'MLDBM',
                        "$PB::Config::RESET_DATA_PATH/validation.db",
                        O_CREAT | O_RDWR, 0660
                    )
                  )
                {
                    $errormessage =
"could not open tie to $PB::Config::RESET_DATA_PATH/validation.db";
                    return $self->forward('error');
                }
...
sub confirm_validation : Runmode {
    ...
    my $html =
      $q->start_html(
        $PB::Config::TEAM_NAME . ' Password Reset: Validation Code');
    ...
    if ($ldap_rval != 0) {
        ...
        $html .= $q->p
          . "User password changed in  $PB::Config::REGISTER_LDAP_O LDAP.";
    }

    # now for the google apps password
    $html .= $q->p
      . "Changing $PB::Config::GOOGLE_DOMAIN user account password for $username";
    my $google_rval = PB::API::google_change_password($username, $password);
    if ($google_rval != 0) {
        $html .= $q->p
          . "Error changing password in google apps for education domain.";
    } else {
        $html .=
          $q->p
          . "User password changed in google apps for education domain ($PB::Config::REGISTER_LDAP_DOMAIN).";
    }
