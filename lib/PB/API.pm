...
my $dbh = DBI->connect(
    'DBI:mysql:database='
      . $PB::Config::PB_DATA_DB_NAME
      . $PB::Config::PB_DEV_VERSION
      . ';host='
      . $PB::Config::PB_DATA_DB_HOST
      . ';port='
      . $PB::Config::PB_DATA_DB_PORT,
    $PB::Config::PB_DATA_DB_USER,
    $PB::Config::PB_DATA_DB_PASS
...
sub ldap_add_user {
    ...
    my $result = $ldap->add(
        "uid=$username,ou=people,$PB::Config::REGISTER_LDAP_DC",
        attr => [
    ...
    my $resultgrp => $ldap->modify(
        "cn=HuntTeam,ou=groups,$PB::Config::REGISTER_LDAP_DC",
        add => {
            memberUid => $username
        }
    );
...
sub ldap_get_user_list {
    ...
    my $mesg = $ldap->search(
        base   => "ou=people,dc=wind-up-birds,dc=org",
        scope  => "sub",
        filter => "sn=*",
...
sub _send_data_version {
    ...
    # Send to meteor
    debug_log(
        "PB::API::_send_data_version() sending version "
          . $dataversion
          . " to meteor\n",
        4
    );
...
sub slack_say_something {
    ...
    print STDERR
      "Running slackcat.py from $PB::Config::PB_GOOGLE_PATH with url $apiurl\n";

    # Prepare command
    my $cmd =
      "./slackcat.py -u $apiurl -c $channel -n $username -t '$message' |";
    my $cmdout = "";
