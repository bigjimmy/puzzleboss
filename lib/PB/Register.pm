...
sub registration : StartRunmode {
    ...
    if ($entriesok) {
        ...
        $html .=
            $q->p
          . "Please verify the registration information before proceeding."
          . $q->start_form(-method => 'POST')
          . $q->dl
          . $q->dt
          . "First Name:"
          . $q->dd
          . $q->textfield(-name => 'firstname', -readonly => 'readonly')
          . $q->dt
          . "Last Name:"
          . $q->dd
          . $q->textfield(-name => 'lastname', -readonly => 'readonly')
          . $q->dt
          . "E-mail Address:"
          . $q->dd
          . $q->textfield(
          ...
          . "Username:"
          . $q->dd
          . $q->textfield(-name => 'username', -readonly => 'readonly')
          . $q->dt
          . "Password:"
          . $q->dd
          . $q->password_field(-name => 'password', -readonly => 'readonly')
          . $q->dt
          . "Re-enter Password:"
          . $q->dd
          . $q->password_field(-name => 'password2', -readonly => 'readonly')
          . $q->p
          . $q->submit(
            -name => 'verifyregister',
            value => 'Everything looks good. Proceed with registration.'
          )
          . $q->p
          . $q->submit(
            -name => 'changes',
            value => 'I want to make changes, please!'
          )
        ...
        $html .=
            $q->p
          . "Please use your real first and last name (unless most people on the team don't know you by that name) and an e-mail address that you can access (for verification)."
          . $q->start_form(-method => 'POST')
          . $q->dl
          . $q->dt
          . "First Name:"
          . $q->dd
          . $q->textfield(-name => 'firstname')
          . $q->dt
          . "Last Name:"
          . $q->dd
          . $q->textfield(-name => 'lastname')
          . $q->dt
          . "E-mail Address:"
          . $q->dd
          ...
          . "Re-enter Password:"
          . $q->dd
          . $q->password_field(-name => 'password2')
          . $q->p
          . $q->submit(-name => 'register', -value => 'Register new account')
          . $q->hidden(-name => 'rm', -value => $nextrm, -override => 1)
          . $q->end_form;
    }
...
sub validation : Runmode {
    ...
    my $q    = $self->query();
    my $html =
      $q->start_html($PB::Config::TEAM_NAME . ' New User Registration');
...
# Better re-validate entries just to be sure...
   ...
   # except first we should untaint the data and protect ourselves one more time
        my $errmsg = "";
        $q->param('firstname') =~ m/^([a-zA-Z]+)$/
          or $errmsg .= "Bad firstname. ";
        my $firstname = $1;
        $q->param('lastname') =~ m/^([a-zA-Z]+)$/
          or $errmsg .= "Bad lastname. ";
        my $lastname = $1;
        $q->param('username') =~ m/^([a-zA-Z0-9]+)$/
          or $errmsg .= "Bad username. ";
        my $username = $1;

# email address first part chars: http://www.remote.org/jochen/mail/info/chars.html
# hostname chars RFC1123
        $q->param('email') =~
          m/^([0-9a-zA-Z&'*+\-\.\/=?^_{}~]+\@[a-z0-9][a-z0-9\-\.]*[a-z0-9])$/
          or $errmsg .= "Bad e-mail. ";
        my $email = $1;
        $q->param('password') =~
          m/^([0-9a-zA-Z\!\#\%\(\)\*\+\,\-\.\/\~\:\=\?\@\[\\\]\^\_\{\}]+)$/
          or $errmsg .= "Bad password. ";
        my $password = $1;
        $q->param('password2') =~
          m/^([0-9a-zA-Z\!\#\%\(\)\*\+\,\-\.\/\~\:\=\?\@\[\\\]\^\_\{\}]+)$/
          or $errmsg .= "Bad password2. ";
        my $password2 = $1;
...
#foreach my $entry ($result->entries) { $html .= "<li>have LDAP entry: [".$entry->ldif."]</li>/"; }
        ...
        if ($matchcount > 0) {
            $html .=
"Username $username already exists!  If you have lost your password, please visit <a href='https://wind-up-birds.org/resetaccount/'> self-serve reset form </a> to reset. ";
        } else {
          ...
          # no duplication, we are ok to proceed to email validate this new user
                ...
                if (
                    !tie(
                        %hash, 'MLDBM',
                        "$PB::Config::REGISTER_DATA_PATH/validation.db",
                        O_CREAT | O_RDWR, 0660
                    )
                  )
                {
                    $errormessage =
"could not open tie to $PB::Config::REGISTER_DATA_PATH/validation.db";
                    return $self->forward('error');
                }
...
EOB
                ...
                if (sendmail %mail) {
                    $html .=
                        $q->p
                      . "A validation code has been sent to your email address: [$email]."
                      . $q->p
                      . "Please check your mail and either follow the link in the email or enter the code here."
                      . $q->start_form(-method => 'POST')
                      . $q->dl
                      . $q->dt
                      . "Validation code:"
                      . $q->dd
                      . $q->textfield(-name => 'validationcode', -size => 40)
                      . $q->p
                      . $q->submit(
                        -name  => 'register',
                        -value => 'Confirm validation code.'
                      )
                      . $q->hidden(
                        -name     => 'rm',
                        -value    => 'confirm_validation',
                        -override => 1
                      ) . $q->end_form;
                } else {
                    $html .= $q->p
                      . "A confirmation problem with this site has prevented us from sending you the confirmation email. Please contact puzzleboss or Benjamin OConnor for assistance. [$Mail::Sendmail::error]";
                }
...
sub confirm_validation : Runmode {
    ...
    my $html =
      $q->start_html(
        $PB::Config::TEAM_NAME . ' User Registration: Validation Code');
    ...
    if (exists($hash{$validation_code})) {
        ...
        $time      = $aref->{timestamp};
        $html .= $q->p
          . "Creating $PB::Config::GOOGLE_DOMAIN user account $username for $firstname $lastname.";
    } else {
        $html .=
            "Sorry, the validation code you entered was not recognized."
          . $q->start_form(-method => 'POST')
          . $q->dl
          . $q->dt
          . "Validation code:"
    ...
    my $ldap_rval =
      PB::API::ldap_add_user($username, $firstname, $lastname, $email,
        $password);
    if ($ldap_rval != 0) {
        $html .= $q->p
          . "<b>ERROR adding user to LDAP. Please report to benoc at alum.mit.edu or puzzleboss<b>";
    } else {
    ...
    my $google_rval =
      PB::API::google_add_user($username, $firstname, $lastname, $password);
    if ($google_rval != 0) {
        $html .= $q->p
          . "<b>ERROR adding user to google apps for education domain. Please report to benoc at alum.mit.edu or puzzleboss</b>";
    } else {
        $html .= $q->p
          . "User added to google apps for education domain ($PB::Config::REGISTER_LDAP_DOMAIN).";
    }
    ...
    if (!($pbdb_rval < 0)) {
        ...
        $html .=
          $q->p
          . "Error adding user to solver database. This is expected if you existed before (e.g. password reset)";
    }

    $html .=
        $q->p
      . 'If all was succesful, you should now be able to login: <a href="http://'
      . $PB::Config::PB_DOMAIN_NAME
      . '">here</a>';
    return ($html);
}
...
sub error : Runmode {
    ...
    $html .=
      $q->start_html($PB::Config::TEAM_NAME . ' User Registration: Error');
...
sub validate_registration_form_data {
    ...
    if ($q->param('firstname') && $q->param('firstname') ne "") {
        if ($q->param('firstname') =~ m/^[^A-Z]/) {
            ...
            $html .= $q->p
              . "First letter of first name must be uppercase (I have fixed this for you).";
        }
        if ($q->param('firstname') =~ m/[^a-zA-Z]/) {
            ...
            $html .= $q->p
              . "Only letters are allowed in first name (no spaces, punctuation, or numbers -- these have been removed).";
        }
    } else {
    ...
    if ($q->param('lastname') && $q->param('lastname') ne "") {
        if ($q->param('lastname') =~ m/^[^A-Z]/) {
            ...
            $html .= $q->p
              . "First letter of last name must be uppercase (I have fixed this for you).";
        }
        if ($q->param('lastname') =~ m/[^a-zA-Z]/) {
            ...
            $q->param('lastname', $ln);
            $html .= $q->p
              . "Only letters are allowed in last name (no spaces, punctuation, or numbers -- these have been removed).";
        }
    } else {
          ...
          )
        {
            ...
            if ($q->param('email') =~ m/^([^@]+)\@([^@]+)$/) {
                ...
                if ($q->param('email') =~ m/\@.*\@/) {
                    ...
                    $html .=
"Did not find an AT-symbol in your e-mail address. Please correct!";
                }
            }
        }
    } else {
        ...
        && $q->param('password2') ne "")
    {
        if ($q->param('password') ne $q->param('password2')) {
            ...
            {
                $html .= $q->p
                  . "Password contains disallowed characters. Please limit your password to the character class [0-9a-zA-Z\!\#\%\(\)\*\+\,\-\.\/\~\:\=\?\@\[\\\]\^\_\{\}]";
                $q->param('password',  "");
                $q->param('password2', "");
            }
        }
