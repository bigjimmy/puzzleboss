...
sub list_POST : Runmode {
    my $self = shift;
    my $errmsg =
      "PB::REST::Puzzles::list_POST: POST to puzzle list is not implemented";
    print STDERR $errmsg;
    $error_status = 501;
    die $errmsg;
}
...
sub part_GET : Runmode {
    ...
    if (   !defined($solverref)
        || (ref($solverref) eq 'ARRAY' && @$solverref == 0)
        || (ref($solverref) eq 'HASH'  && (keys %$solverref) == 0))
    {
    ...
    if (exists($solverref->{$part})) {
        ...
        my $errmsg =
"PB::REST::Solvers::part_GET: could not find part $part in solver $id";
        print STDERR $errmsg;
        $error_status = 404;
        die $errmsg;
    }
...
sub part_POST : Runmode {
    ...
    if (exists($partref->{'data'})) {
        if (PB::API::update_solver_part($solvername, $part, $partref->{'data'})
            < 0)
        {
            my $errmsg =
"PB::REST::Solvers::part_POST: could not update $part for $solvername";
            print STDERR $errmsg;
            $error_status = 404;
            die $errmsg;
        }
        ...
        my $errmsg =
"PB::REST::Solvers::part_POST: did not specify data for solver $id part $part in json $json";
        print STDERR $errmsg;
        $error_status = 400;
        die $errmsg;
    }
