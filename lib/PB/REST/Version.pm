...
sub list_GET : Runmode {
    ...
    if ($version < 0) {
        my $errmsg =
"PB::REST::Version::list_GET: not found (perhaps no data is present in the database?)";
        print STDERR $errmsg;
        $error_status = 404;
        die $errmsg;
    }
...
sub list_POST : Runmode {
    my $self = shift;
    my $errmsg =
      "PB::REST::Version::list_POST: POST to version list is not implemented";
    print STDERR $errmsg;
    $error_status = 501;
    die $errmsg;
}
...
sub full_GET : Runmode {
    ...
    if ($self->query->param('full') && ($self->query->param('full') eq "1")) {
        ...
        if (ref($diffref) ne 'HASH') {
            my $errmsg =
"PB::REST::Version::full_GET: PB::API::get_full_log_diff($current_index): $diffref";
            print STDERR $errmsg;
            $error_status = 404;
            die $errmsg;
        }
        ...
        if (ref($diffref) ne 'HASH') {
            my $errmsg =
"PB::REST::Version::full_GET: PB::API::get_log_diff($current_index): $diffref";
            print STDERR $errmsg;
            $error_status = 404;
            die $errmsg;
        }
...
sub part_GET : Runmode {
    ...
    if ($self->query->param('full') eq "1") {
        ...
        if (ref($diffref) ne 'HASH') {
            my $errmsg =
"PB::REST::Version::part_GET: PB::API::get_full_log_diff($current_index, $to_index): $diffref";
            print STDERR $errmsg;
            $error_status = 404;
            die $errmsg;
        }
        ...
        if (ref($diffref) ne 'HASH') {
            my $errmsg =
"PB::REST::Version::part_GET: PB::API::get_log_diff($current_index, $to_index): $diffref";
            print STDERR $errmsg;
            $error_status = 404;
            die $errmsg;
        }
