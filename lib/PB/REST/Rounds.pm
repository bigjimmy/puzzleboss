...
sub full_GET : Runmode {
    ...
    if (   !defined($roundref)
        || (ref($roundref) eq 'ARRAY' && @$roundref == 0)
        || (ref($roundref) eq 'HASH'  && (keys %$roundref) == 0))
    {
...
sub full_POST : Runmode {
    ...
    print STDERR
      "PB::REST::Rounds::full_POST: request to create round $roundid\n";
    ...
    if ($rval == 0) {
        ...
        my $errmsg =
"could not add $roundid, alphanumeric characters, only, please, and don't duplicate existing rounds";
        print STDERR $errmsg;
        $error_status = 404;
        die $errmsg;
    }
...
sub part_GET : Runmode {
    ...
    if (exists($roundref->{$part})) {
        ...
        my $errmsg =
          "PB::REST::Rounds::part_GET: could not find part $part in round $id";
        print STDERR $errmsg;
        $error_status = 404;
        die $errmsg;
    }
...
sub part_POST : Runmode {
    ...
    if (exists($partref->{'data'})) {
        if (PB::API::update_round_part($id, $part, $partref->{'data'}) < 0) {
            my $errmsg =
              "PB::REST::Rounds::part_POST: could not update $part for $id";
            print STDERR $errmsg;
            $error_status = 404;
            die $errmsg;
        }
        ...
        my $errmsg =
"PB::REST::Rounds::part_POST: did not specify data for round $id part $part in json $json";
        print STDERR $errmsg;
        $error_status = 400;
        die $errmsg;
    }
