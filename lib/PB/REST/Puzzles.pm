...
sub full_GET : Runmode {
    ...
    if (   !defined($puzzleref)
        || (ref($puzzleref) eq 'ARRAY' && @$puzzleref == 0)
        || (ref($puzzleref) eq 'HASH'  && (keys %$puzzleref) == 0))
    {
...
sub part_GET : Runmode {
    ...
    if (exists($puzzleref->{$part})) {
        ...
        my $errmsg =
"PB::REST::Puzzles::part_GET: could not find part $part in puzzle $id";
        print STDERR $errmsg;
        $error_status = 404;
        die $errmsg;
    }
...
sub part_POST : Runmode {
    ...
    if (exists($partref->{'data'})) {
        if (PB::API::update_puzzle_part($id, $part, $partref->{'data'}) < 0) {
            my $errmsg =
              "PB::REST::Puzzles::part_POST: could not update $part for $id";
            print STDERR $errmsg;
            $error_status = 404;
            die $errmsg;
        }
        ...
        my $errmsg =
"PB::REST::Puzzles::part_POST: did not specify data for puzzle $id part $part in json $json";
        print STDERR $errmsg;
        $error_status = 400;
        die $errmsg;
    }
